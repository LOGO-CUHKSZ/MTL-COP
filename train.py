...
if __name__ == "__main__":
    ...
    if trainer_params['cusumucb']['max_nb_random_events'] is None:
        trainer_params['cusumucb']['max_nb_random_events'] = opts.epochs/(2*(1-0.1**2))  # we set alpha=0.1
    ...
    optimizer_params['scheduler']['milestones'] = [900,2900,4900] #if (not trainer_params['separate_train']) or (opts.bandit_alg is not None) else [milestone * len(problem_list),]
